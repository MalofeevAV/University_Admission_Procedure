...
def sorted_list_of_applicants(list_of_applicants):
    """Sort applicants in ascending order by GPA and alphabetical order by their names and surnames"""
    return sorted(list_of_applicants, key=lambda x: (-float(x[2]), x[0] + " " + x[1]))
...
def create_dict_with_applicants(list_of_applicants, max_num_of_students):
    ...
    for i in range(len(list_of_applicants[0][3:])-1):
...
def print_result(dict_with_applicants):
    ...
    for department in sorted(dict_with_applicants):
        print(department)
        for applicant, gpa in dict_with_applicants[department].items():
            print(applicant, gpa)
        print()
